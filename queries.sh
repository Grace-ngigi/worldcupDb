...
echo "$($PSQL "SELECT DISTINCT name FROM teams t FULL JOIN games ls ON t.team_id = ls.winner_id FULL JOIN games le ON t.team_id = le.opponent_id WHERE ls.round = 'Eighth-Final' AND ls.year ='2014' OR le.round ='Eighth-Final' AND le.year = '2014' ORDER BY name")"
